...
{   
    ...
    function createHiddenWord(secretW)  // 7. Creation of a representation of the chosen word, with hidden letters as underscores.                 
{
   let hidden = [];                   
      ...
      for(let i = 0; i < secretW.length; i++)     
   {
    ...
    function displayGameState(hidden, lives, input) // 8. Creation of a function displaying the game's current state to the player.
   { 
    ...
    function isNotLetter(input) // 9. Creation of a function verifying that the player suggested indeed a letter.
{ 
    ...
    function checkLetter(input, secret, hidden, inputList) // 10. Creation of a function verifying that the letter entered matches a letter in the word. 
{
   ...
   while(!isNaN(inputLetterPrompt) || checkInputLetter || inputLetterPrompt.length !== 1) // 7. Checking if the input is a valid letter [not an empty string, a number, or another ASCII character];
    { 
      inputLetterPrompt = prompt("ONE letter ONLY - from the English alphabet. You can do this :] Go:").toUpperCase().trim(); // 8. Capturing the entered letter.   
      checkInputLetter = isNotLetter(inputLetterPrompt); //  9. Calling the function checking again if input is a valid letter.
    }

   if(inputLettersA.includes(inputLetterPrompt)) // 10. If letter has already been entered, informing the player. 
    {
...
mainGameLoop();
    ...
    const endingGame = () => // 12. Creation of an arrow function ending the game.
    { 
        function checkVictory(lives) // 12. Creation of a function verifying if the player won or lost the game.
        { 
            if(lives > 0) 
            {
...
// II. MAIN GAME LOOP 
               ...
               // 2. Informing the player.   !!!!! [Bug: returns undefined (other solution after the 'return')] ?
